...
6. Added optional location to open traffic only from `au` and `tok` locations for `codeengine` network zone and leave the flow closed in other locations.
7. Added optional location to open traffic only from `fr` location for `server-protect` network zone and leave the flow closed in other locations.
