...
| <a name="input_location"></a> [location](#input\_location) | The region in which the network zone is scoped. If no value is passed, serviceRef zone doesn't scope to any location | `string` | `null` | no |
...
| <a name="input_zone_service_ref_list"></a> [zone\_service\_ref\_list](#input\_zone\_service\_ref\_list) | (String) Details of the target service for which the rule has to be created | <pre>map(object({<br>    serviceRef_location = optional(string)<br>  }))</pre> | n/a | yes |
